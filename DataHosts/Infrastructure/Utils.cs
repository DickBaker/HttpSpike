...
namespace Infrastructure
{
    public static class Utils
    {
        ...
        public static string GetRelativePath(string fromPath, string toPath)
        {
            ...
            if (!toPath.StartsWith(@"c:\", StringComparison.InvariantCultureIgnoreCase))        // assume same as base path
            {
                //Console.WriteLine($"not absolute fs\t{toPath}");
            }
            var fromUri = new Uri(fromPath);
            var toUri = new Uri(fromUri, toPath);
            ...
            {
                relativePath = ".";             // TODO: see if all browsers happier with "" or "." for self
            }
