...
{
    public class DownloadParseManager : IDisposable
    {
        ActorSystem allTheWorld;

        ActorSystem AllTheWorld =>
            allTheWorld ?? (allTheWorld = ActorSystem.Create(ActorNames.STAGENAME));

        readonly string OutPath;
        IActorRef DownloadCoordinator;
        IActorRef ParseCoordinator;
        readonly Dictionary<string, TaskCompletionSource<string>> InProgressQ = new Dictionary<string, TaskCompletionSource<string>>();

        public DownloadParseManager(string outPath)
        {
            ...
            ParseCoordinator = AllTheWorld.ActorOf(
                Props.Create(() => new ParseCoordinatorActor(null)), ActorNames.ParseCoordinatorActor.Name);
            DownloadCoordinator = AllTheWorld.ActorOf(
                Props.Create(() => new DownloadCoordinatorActor(null, OutPath)), ActorNames.DownloadCoordinatorActor.Name);
        }

        public void ParseFile(string fileName) =>
            ParseCoordinator.Tell(new Messages.ParseHtmlMessage(fileName));
