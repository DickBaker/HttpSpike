...
4	dependent row SHOULD pre-exist (i.e. PageId will be non-zero), but may need to UPDATE to refresh DraftFilespec, Filespec, NeedDownload columns
...
-- 3.	upsert independent rows as required (NB the dependent row already processed in #2)
  MERGE INTO dbo.WebPages as TGT
		...
		(	-- declare @Url nvarchar(450) = 'http://ligonier.org'
			SELECT	distinct top 100 percent [Url], DraftFilespec, NeedDownload
		...
		WHEN MATCHED THEN
			UPDATE SET
				...
				,	NeedDownload	= isnull(SRC.NeedDownload, TGT.NeedDownload)
